...
class Sam3Processor:
    ...
    def __init__(
        ...
        self.keep_best_when_empty = keep_best_when_empty
    ...
    def _forward_grounding(self, state: Dict):
        ...
        presence_score = outputs["presence_logit_dec"].sigmoid().unsqueeze(1) #[1,1,1] only on element!!!
        out_probs = (out_probs * presence_score).squeeze(-1)

     
        
        keep = out_probs > self.confidence_threshold #[1,200]

        # print(outputs["presence_logit_dec"])
        ########### By Chf ############
        if self.keep_best_when_empty and not keep.any():
            # 在最后一维取 argmax
            max_idx = out_probs.argmax(dim=-1).item()  # 返回整数索引
            # 构造 keep 张量
